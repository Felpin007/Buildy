// src/webview/htmlContent.ts
import * as vscode from 'vscode';
import { getNonce } from '../utils/nonce'; // Assuming nonce util still exists

/**
 * Generates HTML content for the webview, conditionally rendering sections.
 * @param webview The webview instance
 * @param extensionUri The extension URI
 * @param viewType Determines which sections to render ('structure' or 'copySystem')
 */
export function getHtmlForWebview(webview: vscode.Webview, extensionUri: vscode.Uri, viewType: 'structure' | 'copySystem'): string {
    ...
    const scriptUri = webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'media', 'webview', 'main.js')); // We will recreate this
    // Restore CSS links
    ...
    const codiconsUri = webview.asWebviewUri(vscode.Uri.joinPath(extensionUri, 'node_modules', '@vscode/codicons', 'dist', 'codicon.css')); // For button icon
